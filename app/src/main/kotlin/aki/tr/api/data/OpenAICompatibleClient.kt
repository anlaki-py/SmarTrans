...
import aki.tr.api.model.GenerateResult
 ...
 * Uses only `java.net.HttpURLConnection` — no third-party HTTP libraries.
 */
class OpenAICompatibleClient {
    ...
    suspend fun validateKey(
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "SmarTrans")
    ...
    private fun doGenerate(
        ...
        return try {
            ...
            val jsonBody = buildRequestJson(model, prompt, text, temperature)
            ...
            val responseCode = connection.responseCode
            if (responseCode in 200..299) {
                parseSuccessResponse(connection)
            } else {
                parseErrorResponse(connection, responseCode)
            }
        } catch (e: Exception) {
            mapException(e)
        } finally {
    ...
    /**
     * Builds the JSON body for a chat-completions request with structured output.
     */
    ...
    /**
     * Parses a 2xx response into a result string, trying structured JSON first.
     */
    private fun parseSuccessResponse(connection: HttpURLConnection): Result<String> {
    ...
    /**
     * Maps a non-2xx HTTP response to a typed [ApiException].
     */
    private fun parseErrorResponse(
        ...
        if (responseCode == 429) {
            ...
            return Result.failure(ApiException(ApiError.RateLimit(msg, seconds), msg))
        }
        ...
        if (responseCode == 401 || responseCode == 403) {
            ...
            val apiMessage = ApiClientUtils.extractApiErrorMessage(errorBody)
            val detail = if (apiMessage.isNotEmpty()) apiMessage else "Invalid API key"
            return Result.failure(ApiException(ApiError.InvalidKey(detail), detail))
        }
        ...
        return Result.failure(ApiException(apiError, detail))
    }

    /**
     * Wraps a caught exception in an [ApiException] with the appropriate [ApiError] variant.
     */
    private fun mapException(e: Exception): Result<String> {
        if (e is ApiException) return Result.failure(e)

        val apiError: ApiError = when (e) {
            is SocketTimeoutException, is UnknownHostException, is ConnectException ->
                ApiError.Network(e.message ?: "Network error")
            else -> ApiError.Other(e.message ?: "Unknown error")
        }
        return Result.failure(ApiException(apiError, e.message ?: "Unknown error"))
    }
}
