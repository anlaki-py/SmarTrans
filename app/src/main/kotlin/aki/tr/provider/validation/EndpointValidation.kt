package aki.tr.provider.validation
...
object EndpointValidation {
    ...
    fun validate(url: String): EndpointValidationResult {
        ...
        if (url.contains(" ")) return EndpointValidationResult.Error("spaces")
        if (url.startsWith("https://")) return EndpointValidationResult.Valid
        if (url.startsWith("http://")) {
            val host = try { java.net.URL(url).host } catch (_: Exception) { "" }
            return if (host in LOCALHOST_HOSTS) EndpointValidationResult.Valid
            else EndpointValidationResult.Error("scheme")
        }
